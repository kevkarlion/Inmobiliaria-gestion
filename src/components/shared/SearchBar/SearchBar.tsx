  ...
  return (
    <div className="relative w-full mx-auto font-montserrat" ref={containerRef}>
      ...
      <div className="bg-white/95 backdrop-blur-md rounded-2xl md:rounded-full shadow-2xl border border-white/40 p-1 md:p-1.5 flex flex-col md:flex-row items-center gap-1">
        ...
        {/* 1. SELECTORES (Corregido Mobile) */}
        <div className="flex w-full md:w-auto items-center order-1 md:order-2 border-b md:border-b-0 border-gray-100">
          <select
            value={province}
            onChange={(e) => { setProvince(e.target.value); setCity(""); }}
            // CAMBIO: text-[10px] en mobile, font-black para que sea legible pero pequeño
            className="flex-1 md:w-32 lg:w-36 px-2 md:px-3 py-3 bg-transparent text-[10px] md:text-[13px] font-black text-slate-900 outline-none cursor-pointer appearance-none uppercase tracking-tighter"
          >
          ...
          <select
            ...
            className="flex-1 md:w-28 lg:w-32 px-2 md:px-3 py-3 bg-transparent text-[10px] md:text-[13px] font-black text-slate-900 outline-none cursor-pointer appearance-none uppercase tracking-tighter"
          >
        ...
        <div className="relative flex-1 w-full order-2 md:order-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-transparent text-[12px] md:text-sm text-slate-900 placeholder:text-slate-400 font-bold outline-none uppercase tracking-tighter"
      ...
      {/* DROPDOWN - z-[100] para que nada lo tape */}
      {showDropdown && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.2)] z-100 max-h-100 overflow-y-auto border border-slate-100">
           {/* ... resto del contenido del dropdown (ya estaba bien) ... */}
              ...
              filtered.map((p) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-[12px] font-black text-slate-900 uppercase leading-none truncate">{p.title}</h4>
                    <p className="text-[10px] text-slate-400 mt-1 truncate font-medium">{p.cityName}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-[12px] font-black text-slate-900">{p.currency} {p.amount.toLocaleString("es-AR")}</p>
                  </div>
                </Link>
              ))
            ) : (
              <div className="py-10 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">No se hallaron coincidencias</div>
            )}
           </div>
        </div>
      )}
